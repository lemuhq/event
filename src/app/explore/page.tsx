...
const featuredCalendars = [
	{
		...
		description:
			"Not a book club. A reading party. Read with friends to live music & ambient sounds.",
	...
	},
	{
		...
		name: "ADPList",
		description:
			"Your one-stop-shop for all things happening in the ADPList community.",
	...
	},
	{
		...
		location: "Sydney",
		description:
			"The best place in the world to learn AI. Curated with love.",
	...
	},
	{
		...
		name: "Her Workplace",
		description:
			"A community for women in tech to connect, learn, and grow together.",
	...
	},
	{
		...
		name: "Design Studies",
		description:
			"Weekly design workshops and critiques for growing designers.",
...
export default function ExplorePage() {
	...
	const { data: allEvents, isLoading: eventsLoading } = useGetAllEventsQuery(
		{}
	);
	const { data: allEventCategories, isLoading: categoriesLoading } =
		useGetAllEventCategoriesQuery({});
	// console.log("🚀 ~ ExplorePage ~ allEventCategories:", allEventCategories);

	// if (isLoading)
	// 	return (
	// 		<div className="w-screen h-screen fixed top-0 left-0 right-0 bottom-0 bg-[#0A0A0B] z-50 flex items-center justify-center">
	// 			<h2 className="text-white font-semibold text-lg animate-bounce">
	// 				EventWave
	// 			</h2>
	// 		</div>
	// 	);
								...
								>
									<Link href={`/explore/${category.id}`}>
										<div className="bg-neutral-800/50 rounded-xl p-4 hover:bg-neutral-800 transition-all flex items-center gap-4">
											...
											<div>
												<h3 className="font-medium text-white truncate max-w-[32ch]">
													{category.name}
												</h3>
